...
function loadJSON(str,id,override,anim) {
    if (!override) {
      clearCode(id);
    }
    ...
    if (override) {
      console.log("loadJSON: " + anim);
      var json = generateJSObject(id);
      var jc = json.chunks;
      var dist = 5;
      for (var i = 0; i < obj.chunks.length; i++) {
        ...
        for (var j = 0; j < jc.length; j++) {
          if (Math.abs(parseInt(jc[j].x) - x) < dist && Math.abs(parseInt(jc[j].y) - y) < dist) {
            ok = false;
            break;
          }
        }
    ...
    if (anim) {
      //Animate tile load
      var count = tiles2[id].length - anim;
      Array.prototype.forEach.call(tiles2[id], function(e,i) {
        if (i < count) { return; }
        e.lastT = e.style.transition;
        e.style.opacity = 0;
        e.lastLeft = e.style.left;        
        e.style.left = "-2000px";        
      });
      ...
      Array.prototype.forEach.call(tiles2[id], function(e,i) {
        if (i < count) { return; }
        e.style.transition = "opacity 0.3s, left 0.7s ease-out";
        e.style.left = e.lastLeft;
        e.style.opacity = 1;
        delete e.lastLeft;
      });
      },15);
...
function loadSample(k,id) {
    ...
    req.send(null);
    if (req.status == 200) {
        bgMinigrace2[0].onmessage = function(ev) {
            if (!ev.data.success) {
        ...
        bgMinigrace2[0].postMessage({action: "compile", mode: "json",
