...
function changeDialect() {
    ...
    generateCode();
    checkpointSave();
    if (conState == 2) { ws_sendDialectChange(); }
    updateConMenuTitle();
    // if (conMenu != null) { showConMenu(true); }
}
...
function go() {
    ...
    if (mgrunning) { mgstop = true; return; }
    ...
    var txt = document.getElementById('stdout_txt');
    var out = document.getElementById('stderr_txt').value.split("\n");
    if (out[out.length-2] != "minigrace: main: done.") {
      runButton.style.background = "red";
      
      if (showErrorsInOutputArea) {
        for (var i = 0; i < out.length; i++) {
          if (!out[i].startsWith("minigrace:") && out[i] != "") {
            txt.value += out[i] + "\n";
          }
        }      
      }
