...
function isBottomTarget(ch, obj) {
    if (ch.collapsed) { return false; }
    if (obj.collapsed) { return false; }
...
function dragstart(ev) {    
    ...
    if (ev.target && ev.target.classList && ev.target.classList.contains("collapsers")) { return; }
    ...
    var dragcontinue = function(ev2) {
        ...
        if (bestHole != null && bestHole.children.length == 0 && !obj.collapsed) {
    ...
    var dragend = function(ev) {
        ...
        if (bestHole != null && !obj.collapsed) {
        ...
        if (obj.collapsed) { bestHole = null; }
        for (var i=0; i<tiles.length; i++) {
            var ch = tiles[i];
            ch.classList.remove('bottom-join-target');
            if (bestHole)
